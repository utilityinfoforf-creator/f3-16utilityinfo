...
const FALLBACK_APP_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbzUQeJKLBRCjQG928fnIdJ7Tlg-JR0072ENK-K2_07NBOxWsH9zs0qd5CrcoQW_Mbz3lA/exec';
const API_BASE = (window.APP_SCRIPT_URL || window.API_URL || window.API_BASE || window.__API_URL__ || window.__API_BASE__ || localStorage.getItem('APP_SCRIPT_URL') || FALLBACK_APP_SCRIPT_URL).trim();

function getAppScriptUrl() {
  const configured = (window.APP_SCRIPT_URL || window.API_URL || window.API_BASE || window.__API_URL__ || window.__API_BASE__ || localStorage.getItem('APP_SCRIPT_URL') || FALLBACK_APP_SCRIPT_URL).trim();
  if (!configured) {
...
async function exportUsageReport() {
  ...
  try {
    ...
    if (data.usage && data.usage.length > 0) {
      ...
      csv += `${id},${data.customerName || name},${new Date().toLocaleDateString()},Pending,No data available\n`;
    }
